# anchor-french 配置架构与数据接口规范

> 本技能包的权威 schema 与接口文档，随包分发、按需查阅。
>
> - 配置数据：`assets/fr-corner-config.json`（唯一数据源）
...
>
> **本包只服务法语。** 其中不含任何其它语言的配置、分支或文案；其它语言角是各自独立的技能包，彼此不共享文件、不做交叉校验。
...
- **单一结构真源**：所有可选项、提问编排、风格与文案都在 `assets/fr-corner-config.json`，增删选项只改 JSON，`SKILL.md` 无需改动；
- **单一解析层**：`scripts/corner_config.py` 只做「JSON → `SkillConfig` + 查询 API」，展示文案全部来自 `style.i18n`，模块内不硬编码任何文案；
- **语言专一**：本包只认 `fr-corner-config.json`，函数签名里没有 `lang` 参数，也不做多语言探测——误读到别的语言配置本身就是错误；
...
| `exam`               | object                            | `levels` / `rules` / `rubric[]`：本包的评分体系对照（法语 DELF/DALF）                                                                                |
| `question_plan`      | array[QuestionSpec]               | **交互问题编排**（驱动 intake 状态机）                                                                                                               |
| `style`              | object                            | 风格约定：`pure_french` / `no_hr` / `no_full_line_bold` / `output_path_template` / `pos_groups[]` / `phase_labels{}` / `section_labels{}` / `i18n{}` |

> `brief_filename`（meta 内）：简报落盘文件名（`fr-corner-brief.md`）；`export_brief()` 据此决定输出名，不硬编码。
...
### 1.3 `question_plan`（intake 状态机蓝图）
...
- `path=None`：固定读取 `assets_dir() / DEFAULT_CONFIG_NAME`，即 `<skill>/assets/fr-corner-config.json`，不做任何探测或回退；
...
cfg = load_config()  # 读 assets/fr-corner-config.json
...
### 3.3 导出简报（交给 skill 主体读取，不依赖 LLM / openai）
...
| `assets/fr-corner-config.json`        | 法语选择数据（唯一数据源）                                                                                    |
...
```bash
python scripts/corner_config.py                        # 加载 + 校验 assets/fr-corner-config.json
python scripts/corner_config.py assets/fr-corner-config.json   # 显式指定配置（等价）
python scripts/corner_skill.py                         # intake 状态机 + 简报导出
...
2. **身份**：`assets/` 下恰好一个 `*-corner-config.json` 且名为 `fr-corner-config.json`；`SKILL.md` frontmatter `name` == `SKILL_NAME` == `meta.skill_name`；
...
| 项                          | 本包取值                                                                                                                             |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `meta.skill_name`           | `anchor-french`                                                                                                                      |
| `meta.source_of_truth`      | `skills/anchor-french/assets/fr-corner-config.json`（JSON 自身为唯一数据源）                                                         |
| `meta.brief_filename`       | `fr-corner-brief.md`                                                                                                                 |
| `grammar_points`（一级 id） | `adj_adv` / `indicatif` / `pronoms` / `relatives_conj` / `modes` / `nonfinis_passif` / `fragments`（各含法语二级条目）               |
| `exam.levels`               | `DELF B1` / `DELF B2` / `DALF C1` / `DALF C2`                                                                                        |
| `question_plan`             | Q1 一级语法点 → Q2 二级条目 → Q3 水平 → **Q4 话题 `ask:true, source:"topic_pool", allow_random`**（选题库 / 随机 / 自定义）→ Q5 规模 |
| `style.pure_french`         | `true`（产出语言：法语）                                                                                                             |
| `style.phase_labels`        | `Ouverture` / `Brise-glace` / `Thème` / `Discussion` / `Clôture`                                                                     |
| `style.pos_groups`          | `Noms` / `Verbes` / `Adjectifs` / `Adverbes` / `Prépositions·Conjonctions` / `Pronoms` / `Autres`                                    |
| `style.i18n`                | 所有简报展示文案（标题、约束、已收集参数、产出要求、rubric 行等）均为法语，由解析层统一渲染                                          |
