---
name: project-tex
description: LaTeX 数学公式的写法规范。写、改、审查公式时套用——覆盖行内与独立公式、多行推导、矩阵与行列式、方程组与分段函数、括号尺寸、转置/极限/argmin 记号、正体粗体斜体字体命令；落笔后用 scripts/check_style.py 复查。触发词：LaTeX、latex、tex、公式、数学公式、矩阵、行列式、方程组、分段函数、begin、aligned、bmatrix、vmatrix、mathrm、atop。
---
...
id 与脚本里的规则同名同义，两边改动要一起改。`--selfcheck`（旧名 `--list-rules`）双向核对：既报「只进了脚本、没进本表」的 id，也报「只在本表、脚本里没有」的 id；两个方向都干净才退出 0。
...
没有单独的自检脚本，入口就是检查脚本自己：

```bash
python <this skill dir>/scripts/check_style.py --selfcheck
```

它打印 `RULES` 里的九条规则，再拿它们与本文件的 `## 规则表` 双向对账 —— 只进了脚本、没进表的 id，只在表里、脚本里没有实现的 id，两个方向都干净才退出 0。`--list-rules` 是同一个开关的旧名。
